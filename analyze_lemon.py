...
# Load the data
with open('yt_lemon_juice_1777110237405.json', 'r') as f:
    data = json.load(f)

videos = data['videos']

print("=" * 80)
print("LEMON JUICE YOUTUBE ANALYSIS - MARKETING INSIGHTS")
print("=" * 80)
...
title_keywords = [
    'liver', 'kidney', 'weight loss', 'detox', 'cleanse', 'skin', 'acid', 'alkaline',
    'health', 'benefits', 'mistakes', 'vitamin c', 'immune', 'digestion', 'stomach',
    'morning', 'water', 'olive oil', 'kidney stone', 'ph', 'weight', 'lose'
]
...
health_claims = [
    ...
    ('stomach', r'stomach'),
    ('morning', r'morning'),
    ('empty stomach', r'empty\s*stomach'),
    ('ph', r'\bpH\b'),
    ('hydration', r'hydrat'),
...
compounds = ['vitamin c', 'citric acid', 'citrate', 'potassium', 'magnesium', 
             'calcium', 'flavonoid', 'antioxidant', 'limonene', 'fiber', 'pectin']
...
# 6. Lemon + Ginger + Beetroot Content
print("\n" + "=" * 80)
print("6. LEMON COMBINATION CONTENT")
print("=" * 80)

lemon_combo_videos = {'ginger': [], 'beetroot': [], 'honey': []}
for v in videos:
    title_lower = v['title'].lower()
    transcript_text = ""
    if v.get('transcript') and v['transcript'].get('fullText'):
    ...
    if 'ginger' in title_lower or 'ginger' in transcript_text:
        lemon_combo_videos['ginger'].append(v)
    if 'beet' in title_lower or 'beetroot' in title_lower or 'beet' in transcript_text:
        lemon_combo_videos['beetroot'].append(v)
    if 'honey' in title_lower or 'honey' in transcript_text:
        lemon_combo_videos['honey'].append(v)

for combo, vids in lemon_combo_videos.items():
    print(f"\nLemon + {combo.upper()}: {len(vids)} videos")
...
key_claim_patterns = [
    (r'vitamin\s*c\s+(helps?|boosts?|support|is)', 'vitamin c benefits'),
    (r'(liver|kidney)\s+(cleanse|detox|health|support)', 'organ health'),
    (r'(improves?|boosts?|helps?)\s+(digestion|immune|skin)', 'health benefits'),
    (r'(kidney\s*stone)', 'kidney stones'),
    (r'(morning|empty\s*stomach)', 'timing'),
    (r'\d+\s*(%|percent|mg|gram|cup)', 'statistics'),
]
...
audiences = {
    'Morning Routine': ['morning', 'empty stomach', 'wake up', 'daily', 'every day'],
    'Liver Health': ['liver', 'cleanse', 'detox'],
    'Kidney Health': ['kidney', 'stone'],
    'Weight Management': ['weight loss', 'lose weight', 'fat', 'metabolism'],
    'Skin/Beauty': ['skin', 'beauty', 'face'],
    'Digestive Health': ['digestion', 'stomach', 'gut', 'acid'],
    'Immune Support': ['immune', 'vitamin c', 'cold'],
    'Alkaline/pH Balance': ['alkaline', 'ph', 'acid'],
}
...
lemon_report = {
    ...
    'combo_videos': {k: [{'title': v['title'], 'views': v['views']} for v in vids] 
                     for k, vids in lemon_combo_videos.items()}
}

with open('lemon_analysis_report.json', 'w') as f:
    json.dump(lemon_report, f, indent=2)

print("Lemon analysis saved to lemon_analysis_report.json")
print("\n" + "=" * 80)
print("LEMON ANALYSIS COMPLETE!")
print("=" * 80)